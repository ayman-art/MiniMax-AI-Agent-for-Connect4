import pygame as pg
from GUI.board import Board
from GUI.utils import Config
import pygame_widgets
from pygame_widgets.dropdown import Dropdown
from pygame_widgets.button import Button
from pygame_widgets.slider import Slider
from pygame_widgets.textbox import TextBox

def draw_label(font, sur, text, pos):
    label = font.render(text, 1, (255,255,0))
    sur.blit(label, pos)
class Window:
    def reset_turn(self):
            self.turn = "-"
            self.started = False
    def player_callback(self):
            self.started = True
            self.turn = "Agent"
            
            # call score calc function
            self.start_buttonHandler()
    
    def start_buttonHandler(self):
            mode = self.dropdown.getSelected()
            if mode is None:
                pass
            k = int(self.output.getText())
            if self.started == False or self.turn == "Agent":
                self.started = True
                self.turn = "Agent"
                val = 1 # Add agent logic here
                self.board.drop_piece_in_column(val, 1)
                self.turn = "Player"
                # call score calc function

    def __init__(self):
        
        pg.init()
        self.size = width, height = 1440, 900
        self.label_font = pg.font.SysFont("monospace", 20)
        self.window = pg.display.set_mode(self.size)
        pg.display.set_caption("Connect 4 Agent")
        self.clock = pg.time.Clock()
        running = True
        self.mat = [[0 for _ in range(7)] for i in range(6)]
        self.board = Board(self.mat, 20, 20)
        self.dropdown = Dropdown(self.window, 20, 20, 200, 50, name="Select Algorithm",
                            choices=["Minimax", "Minimax-Pruning", "Expected Minimax"],
                            borderRadius= 1, colour=Config.GREEN, values=[1, 2, 3], direction='down', textHAlign='centre'
                            )
        self.button = Button(self.window, 300, 20, 100, 50, text="AI Start",
                              onClick=self.start_buttonHandler, pressedColour=Config.GREEN)
        self.slider = Slider(self.window, 450, 50, 200, 40, min=1, max=42, step=1, initial=1, handleColour=Config.BLUE)
        self.output = TextBox(self.window, 537, 20, 25, 25, fontSize=15)
        self.output.disable()
        self.p1_score = ""
        self.p2_score = ""
        self.turn = "-"
        self.started = False


        while running:
            self.clock.tick(Config.FPS)
            self.window.fill(Config.BLACK)
            events = pg.event.get()
            for event in events:
                if event.type == pg.QUIT:
                    running = False
                self.board.handle_event(event, self.turn, self.player_callback)
            self.board.draw(self.window)
            pygame_widgets.update(events)
            draw_label(self.label_font, self.window, "P1 Score: "+ str(self.p1_score), (20, 800))
            draw_label(self.label_font, self.window, "P2 Score: "+ str(self.p1_score), (20, 850))
            draw_label(self.label_font, self.window, "Turn: "+ self.turn, (350, 825))
            self.output.setText(self.slider.getValue())
            pg.display.update()
        pg.quit()

